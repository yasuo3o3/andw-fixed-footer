 ...
 * プラグインが作成したデータを削除
 */
function andwff_uninstall_cleanup() {
    // 管理者権限チェック
    if (!current_user_can('delete_plugins')) {
        return;
    }

    // プラグイン設定オプションを削除（新旧両方）
    delete_option('andwff_options');
    delete_option('andw_fixed_footer_options'); // Legacy support
    ...
    andwff_delete_transients();
}

/**
 * 当プラグイン関連のtransientsを削除
 */
function andwff_delete_transients() {
 ...
 */
function andwff_delete_cache() {
    // 外部オブジェクトキャッシュ（Redis、Memcached等）を使用している場合の対応
    if (function_exists('wp_cache_supports') && wp_cache_supports('flush_group')) {
        wp_cache_flush_group('andwff');
        wp_cache_flush_group('andw_fixed_footer'); // Legacy support
    }
    ...
    foreach ($cache_keys as $key) {
        wp_cache_delete($key, 'andwff');
        wp_cache_delete($key, 'andw_fixed_footer'); // Legacy support
    }
}
 ...
 */
function andwff_log_uninstall() {
    // ログ出力なし（WordPress.org審査対応）
}

// アンインストール処理を実行
try {
    andwff_uninstall_cleanup();
    andwff_delete_cache();
    andwff_log_uninstall();
} catch (Exception $e) {
